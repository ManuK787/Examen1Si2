...
export default function Sidebar({ collapsed = false, onToggle }) {
  ...
  const linkBase =
    "flex items-center gap-2 px-4 py-2 rounded transition-colors text-sm font-medium text-white hover:bg-white/10";
  const labelBase =
    "whitespace-nowrap origin-left transition-all duration-200 text-white";
  ...
  return (
    <aside
      className={`group relative ${
        collapsed ? "w-16" : "w-60"
      } bg-gradient-to-b from-slate-900 via-slate-900 to-slate-800 text-white border-r border-slate-800 flex flex-col pt-4 h-screen overflow-y-auto overflow-x-hidden transition-[width] duration-300`}
    >
      <div
      ...
      >
        ...
        <button
          onClick={onToggle}
          className={`rounded-md border border-slate-600 hover:bg-white/10 text-white flex items-center justify-center transition-colors ${
      ...
      <nav className="flex flex-col gap-1 flex-1">
        ...
        >
          ...
          <span
            className={`${labelBase} ${
              collapsed ? labelHidden : labelVisible
            } overflow-hidden`}
          >
            Dashboard
          </span>
      ...
      <div className="mt-auto px-2 pb-4">
        <button
          ...
          } gap-2 py-2 rounded bg-red-600 hover:bg-red-500 text-white text-sm font-medium transition-colors`}
        >
          <LogOut size={18} className="shrink-0" />
          {!collapsed && <span>Cerrar sesión</span>}
        </button>
        {!collapsed && user && (
          <p className="mt-2 text-[11px] text-white/60 px-1 truncate">
            {user.name}
          </p>
        )}
      </div>
